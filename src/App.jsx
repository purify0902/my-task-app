if (!window.storage) {
  window.storage = {
    ...
    delete: async (k) => { localStorage.removeItem(k); return true; },
...
export default function App() {
    ...
    (async()=>{
      try {
        ...
        if(res&&res.value){
          ...
          const maxId=Math.max(...merged.map(t=>t.id),8);
          setTasks(merged); setNextId(Math.max(d.nextId||9,maxId+1));
          if(d.cats) setCats(d.cats);
        } else { setTasks(INIT); setNextId(9); }
      } catch(e){
        try{const s=localStorage.getItem(KEY);if(s){const d=JSON.parse(s);setTasks(d.tasks||INIT);setNextId(d.nextId||9);if(d.cats)setCats(d.cats);}else setTasks(INIT);}catch(e2){setTasks(INIT);}
  ...
  useEffect(()=>{
    (async()=>{
      try{ const res=await window.storage.get(CAT_KEY); if(res&&res.value) setCats(JSON.parse(res.value)); }
      catch(e){}
    })();
  },[]);
  ...
  useEffect(()=>{
    ...
    const timer=setTimeout(async()=>{
      try{ await window.storage.set(KEY,JSON.stringify({tasks,nextId,cats})); setSaveStatus("saved"); }
      catch(e){ try{localStorage.setItem(KEY,JSON.stringify({tasks,nextId,cats}));}catch(e2){} setSaveStatus("local"); }
    },600);
    return ()=>clearTimeout(timer);
  },[tasks,nextId,cats,loading]);
